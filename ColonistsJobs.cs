...
{
    public partial class ColonistsJobs : Form
    {
        ...
        private void LoadUsers()
        {
            ...
            {
                conn.Open();
                SqlDataAdapter da = new SqlDataAdapter("SELECT * FROM ColonistJob", conn);
        ...
        private void button2_Click(object sender, EventArgs e)
        {
            ...
            {
                conn.Open();
                SqlCommand cmd = new SqlCommand("INSERT INTO ColonistJob (Colonization_ID, Jobs_ID) VALUES (@Colonization_ID, @Jobs_ID)", conn);
                cmd.Parameters.AddWithValue("@Colonization_ID", textBox1.Text);
                cmd.Parameters.AddWithValue("@Jobs_ID", textBox2.Text);
                cmd.ExecuteNonQuery();
            ...
            if (dataGridView1.SelectedRows.Count > 0)
            {
                try
                {
                    ...
                    using (SqlConnection conn = new SqlConnection(connectionString))
                    {
                        ...
                        SqlCommand cmd = new SqlCommand("DELETE FROM ColonistJob WHERE Colonization_ID = @Colonization_ID", conn);
