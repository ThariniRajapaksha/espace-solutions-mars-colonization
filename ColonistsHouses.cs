...
{
    public partial class ColonistsHouses : Form
    {
        ...
        private void LoadUsers()
        {
            ...
            {
                conn.Open();
                SqlDataAdapter da = new SqlDataAdapter("SELECT * FROM HouseAssignment", conn);
                DataTable dt = new DataTable();
                da.Fill(dt);
                dataGridView1.DataSource = dt;
            }
        }
        private void ColonistsHouses_Load(object sender, EventArgs e)
        {
        ...
        private void button2_Click(object sender, EventArgs e)
        {
            ...
            {
                conn.Open();
                SqlCommand cmd = new SqlCommand("INSERT INTO HouseAssignment (Colonization_ID, ColonyLotNumber) VALUES (@Colonization_ID, @ColonyLotNumber)", conn);
                cmd.Parameters.AddWithValue("@Colonization_ID", textBox1.Text);
                cmd.Parameters.AddWithValue("@ColonyLotNumber", textBox2.Text);
                cmd.ExecuteNonQuery();
            ...
            if (dataGridView1.SelectedRows.Count > 0)
            {
                try
                {
                    ...
                    using (SqlConnection conn = new SqlConnection(connectionString))
                    {
                        ...
                        SqlCommand cmd = new SqlCommand("DELETE FROM HouseAssignment WHERE Colonization_ID = @Colonization_ID", conn);
